...
const LoginScreen = () => {
  ...
  async function  handleLogin(e){
    e.preventDefault();
    console.log("function calleddddd internalllyyy") 
    ...
    if(returnedvalue){
      ...
      let userid = returnedvalue.user.id
      console.log({userid})
    ...
    <div className="flex items-center justify-center min-h-screen bg-gray-100">
      <div className="w-full max-w-md p-8 space-y-6 bg-white shadow-md rounded-lg">
        <div className="flex justify-center mb-4">
          <button
            className={`px-4 py-2 mr-2 font-medium ${activeTab === 'login' ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-700'}`}
            onClick={() => setActiveTab('login') & setEmail('') & setPassword('') & setConfirmPassword('')}
          >
            Log In
          </button>
          <button
            className={`px-4 py-2 font-medium ${activeTab === 'createUser' ? 'bg-indigo-500 text-white' : 'bg-gray-200 text-gray-700'}`}
            onClick={() => setActiveTab('createUser') & setEmail('') & setPassword('') & setConfirmPassword('')}
          >
            Create User
          </button>
        </div>
