...
const Cart = ({ cartItems, handleAddToCart, handleRemoveFromCart, handleCloseCart, table_id }) => {
      ...
      {cartEntries.length > 0 && (
        <div className="mt-8 border-t pt-4">
          <h3 className="text-xl font-semibold">
            Total: <span className="font-bold">${cartEntries.reduce((total, item) => total + item.price * item.quantity, 0).toFixed(2)}</span>
          </h3>
        </div>
      )}
      ...
      {showConfirmation && (
        <div className="fixed inset-0 flex items-center justify-center bg-gray-800 bg-opacity-50 z-50">
          <div className="bg-white p-6 rounded-lg shadow-lg text-center max-w-md mx-auto">
            <h3 className="text-xl font-semibold mb-4">Are you sure you want to place the order?</h3>
