...
// styles written in AgentHome.scss
...
export default function ConfirmTableScreen({ loc_id, onCountChange }) {
	...
	return (
		<div className="flexbox flex-col items-center justify-center min-h-screen space-y-6 confirmation-table">
			<h1 className="confirmation-table--header">New Table Confirmation</h1>
			{tableRecords.length > 0 ? (
				tableRecords.map((record, index) => (
					<div
						key={record.id}
						className="w-11/12 max-w-md p-2 table-card relative"
					>
						<button
						...
						<p className="text-lg text-center mt-2">
							to Table Number {record.table_id}
						</p>
					</div>
				))
			) : (
