...
import { addmenuitem, getdetailedmenu, loadlocation } from '../services/supported_api';
...
const AddItemForm = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const parsedMenus = typeof neededData.menus === 'string' ? JSON.parse(neededData.menus) : neededData.menus;
  ...
  const uploadImageToImgbb = async (base64Image) => {
    ...
    const response = await fetch('https://api.imgbb.com/1/upload?key=eba7a098c00b0b6007f4820231498515', {
      method: 'POST',
      body: formData,
    });
  ...
  return (
    <div className="flex justify-center items-start min-h-screen bg-gray-100">
      <div className="container mx-auto max-w-4xl px-4 py-6">
        <h1 className="text-2xl font-semibold text-gray-800 text-center mb-8">Add Your Menus</h1>
        <div className="bg-white p-6 rounded-lg shadow-lg max-w-3xl mx-auto overflow-y-auto max-h-[80vh] custom-scrollbar">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              <label className="block text-sm font-medium text-gray-700">Vegetarian or Non-Vegetarian</label>
              <select
                name="veg_or_nonveg"
