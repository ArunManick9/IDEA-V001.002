...
export default function OrdersPage() {
  ...
  const renderOrders = (bucket, fromBucket, toBucket, newStatus) => (
    <ul>
      {buckets[bucket].length === 0 ? (
        <li className="text-center text-gray-600">No orders in this bucket...</li>
      ) : (
        buckets[bucket].map((order) => (
          <li
            ...
            className={`relative mb-4 p-4 border rounded-lg transition-transform transform hover:shadow-lg bg-white ${
              highlightedOrderId === order.order_id ? 'border-blue-500' : 'border-gray-300'
            }`}
          >
            <div className="flex flex-col space-y-1">
              <div className="flex justify-between items-center">
                <p className="text-md font-semibold text-gray-800">Order ID: {order.order_id}</p>
                <div className="text-gray-700 font-semibold">Table: {order.table_id}</div>
              </div>
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4">
      <h1 className="text-2xl font-bold text-center mb-4 text-gray-800">Orders for Location: {locid}</h1>
      ...
      <div className="flex justify-around mb-4">
        {['newOrders', 'acceptedOrders', 'fulfilledOrders'].map((tab) => (
          <button
            key={tab}
            className={`flex-1 p-2 text-center ${activeTab === tab ? 'bg-blue-500 text-white' : 'bg-gray-200 text-gray-700'}`}
            onClick={() => setActiveTab(tab)}
          >
            {tab.charAt(0).toUpperCase() + tab.slice(1).replace('Orders', ' Orders')}
          </button>
        ))}
      </div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md">
        <h2 className="text-lg font-semibold mb-2 text-gray-800">
          {activeTab.charAt(0).toUpperCase() + activeTab.slice(1).replace('Orders', ' Orders')}
        </h2>
        {renderOrders(activeTab, activeTab, activeTab === 'newOrders' ? 'acceptedOrders' : activeTab === 'acceptedOrders' ? 'fulfilledOrders' : null, activeTab === 'newOrders' ? 'Accepted' : activeTab === 'acceptedOrders' ? 'Fulfilled' : null)}
      </div>
    </div>
  );
}
